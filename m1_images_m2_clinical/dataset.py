...
class MultimodalDataProcessor(Dataset):
    ...
    def __getitem__(self, i):
        ...
        image_processed = self.load_and_process_image(img_file, x_pixel, y_pixel, z_pixel)

        if self.to_transform:
            image_processed = self.transform_img(image_processed)

        semantic_text_root = '/workspace/lottie_clip_data/report_generation'
        lesion_index = self.df['nodule_id'][i]
        file_name = os.path.join(semantic_text_root, str(patient_id) + '_' + str(lesion_index) + '.txt')
        # Read the file
        with open(file_name, "r") as file:
            content = file.read()

        index = content.find('Impression')
        texts = content[index + len('Impression') + 1:]
        semantic = longclip.tokenize(texts)

        return {"ct_image": image_processed, 'semantic': semantic, 'label':label, 'patient_id': patient_id}
