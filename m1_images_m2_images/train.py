        ...
        for epoch in range(self.epoch_num):
            if early_stop_count < self.early_stopping_epoch:
                ...
                for index, data in enumerate(train_loader):
                    m1_image, m2_image = data['m1_image'].to(self.device, dtype=torch.float), data['m2_image'].to(self.device, dtype=torch.float)
                    self.optimizer.zero_grad()
                    o1, o2 = self.model(m1_image, m2_image)
    ...
    def finetune(self, 
        ...
        for epoch in range(self.epoch_num):
            if early_stop_count < self.early_stopping_epoch:
                ...
                self.model.train()
                for index, data in enumerate(train_loader):
                    m1_image, m2_image, label, patient_id = data['m1_image'].to(self.device, dtype=torch.float), data['m2_image'].to(self.device, dtype=torch.float), data['label'].to(self.device, dtype=torch.float), data['patient_id']

                    self.optimizer.zero_grad()
                    _, _, pred_logits = self.model(m1_image, m2_image)
